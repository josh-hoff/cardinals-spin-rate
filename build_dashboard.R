...
build_cardinals_dashboard <- function(
  ...
  writeLines('pitchers.forEach(p=>{const s=getPitcherStatus(p);const key=s==="40man"?"fortyMan":s;if(grouped[key])grouped[key].push(p);else grouped.former.push(p);});', out)
