...
build_cardinals_dashboard <- function(
  ...
  cols <- c("player_name","pitch_name","inning_numeric","inning","outs_when_up",
            ...
            "game_pk","outing_progression","opponent","home_away","events")
  df <- df[, cols]
  n_pitches  <- nrow(df)
  ...
  writeLines('.cw{position:relative;width:100%;height:420px;margin-bottom:.5rem}', out)
  ...
  writeLines('window.CARDINALS_DATA.forEach(r=>{if(!BY_PITCHER[r[0]])BY_PITCHER[r[0]]=[];BY_PITCHER[r[0]].push(r);});', out)
  ...
  writeLines('pitchers.forEach(p=>{const o=document.createElement("option");o.value=p;o.textContent=DISPLAY[p]||p;pSel.appendChild(o);});', out)
