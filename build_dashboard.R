...
build_cardinals_dashboard <- function(
  ...
  writeLines('console.log("ROSTER_STATUS:",window.ROSTER_STATUS);console.log("PITCHER_ID_MAP:",PITCHER_ID_MAP);', out)
  ...
  writeLines('function getPitcherStatus(p){const idRaw=PITCHER_ID_MAP[p];if(!idRaw)return"former";const id=Number(idRaw);const rs=window.ROSTER_STATUS;const activeNums=(rs.active||[]).map(Number);const injuredNums=(rs.injured||[]).map(Number);const fortyNums=(rs.forty_man||[]).map(Number);if(activeNums.includes(id))return"active";if(injuredNums.includes(id))return"injured";if(fortyNums.includes(id))return"fortyMan";return"former";}', out)
  ...
  writeLines('const STATUS_BG={active:"#ffffff",injured:"#ffd6d6",fortyMan:"#d6deff",former:"#e8e8e8"};', out)
  writeLines('const STATUS_COLOR={active:"#0b0b0b",injured:"#7a1f1f",fortyMan:"#1a2d5a",former:"#4a4a4a"};', out)
  writeLines('const grouped={active:[],injured:[],fortyMan:[],former:[]};', out)
  writeLines('pitchers.forEach(p=>{const s=getPitcherStatus(p);const key=s==="40man"?"fortyMan":s;grouped[key].push(p);});', out)
  writeLines('function addPitcherOpts(list,status){list.forEach(p=>{const o=document.createElement("option");o.value=p;o.textContent=DISPLAY[p]||p;o.style.background=STATUS_BG[status];o.style.color=STATUS_COLOR[status];pSel.appendChild(o);});}', out)
